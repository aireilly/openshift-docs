...
function get_vale_errors {
    echo "Getting the Vale errors and filtering out existing comments..."
    local vale_json="$1"
    local pull_comments_json="$2"

    # jq map and filter for new review comments only
...
do
    ...
    #Get comments in PR
    pull_comments_json=$(curl -L -H "Accept: application/vnd.github+json" -H "Authorization: Bearer $GITHUB_AUTH_TOKEN" -H "X-GitHub-Api-Version: 2022-11-28" https://api.github.com/repos/$REPO_OWNER/$REPO_NAME/pulls/$PULL_NUMBER/comments | jq)
    ...
    jq -c '.[]' <<< "$updated_comments_json" | while IFS= read -r object; do
        #echo "$object"
