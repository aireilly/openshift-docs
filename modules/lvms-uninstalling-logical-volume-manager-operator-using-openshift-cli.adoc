...
+
--
`$ oc delete clusterserviceversion <currentCSV> -n <namespace>`:: Replace `<currentCSV>` with the `currentCSV` value for the {lvms} Operator. + .Example output [source,terminal] ---- clusterserviceversion.operators.coreos.com "lvms-operator.v4.15.3" deleted ----
--
