...
+
--
`deviceClasses:`:: Contains the configuration to assign the local storage devices to the LVM volume groups.
`nodeSelector:`:: Contains the configuration to choose the nodes on which you want to create the LVM volume group. If this field is empty, all nodes without no-schedule taints are considered.
`deviceSelector:`:: Contains the configuration to specify the paths to the devices that you want to add to the LVM volume group, and force wipe the devices that are added to the LVM volume group.
...
`deviceClassStatuses`:: The status of the device class.
...
`node`:: The node on which the device class is created.
`status`:: The status of the LVM volume group on the node.
`state`:: The status of the `LVMCluster` CR.
+
[NOTE]
====
