...
+
--
`clusterSelector:`:: Set the `key` field and `values` field in `PlacementRule.spec.clusterSelector` to match the labels that are configured in the clusters on which you want to install {lvms}.
`objectDefinition:`:: Namespace configuration.
`objectDefinition:`:: The `OperatorGroup` CR configuration.
`objectDefinition:`:: The `Subscription` CR configuration.
--
