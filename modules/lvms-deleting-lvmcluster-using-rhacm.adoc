...
+
--
`$ oc delete -f <file_name> -n <cluster_namespace>`:: Namespace of the {product-title} cluster on which {lvms} is installed.
--
...
+
--
`remediationAction: enforce`:: The `spec.remediationAction` in `policy-template` is overridden by the preceding parameter value for `spec.remediationAction`.
`namespace: openshift-lvm-storage`:: This `namespace` field must have the `openshift-lvm-storage` value.
`clusterSelector:`:: Configure the requirements to select the clusters. {lvms} is uninstalled on the clusters that match the selection criteria.
--
...
`namespace: openshift-lvm-storage`:: The `namespace` field must have the `openshift-lvm-storage` value.
--
