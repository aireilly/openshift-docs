...
+
--
`storageClassName: lvms-vg1`:: Set this field to the value of the `storageClassName` field in the source PVC.
`volumeMode: Filesystem`:: Set this field to the `volumeMode` field in the source PVC.
`name: lvm-pvc`:: Specify the name of the source PVC.
`storage: 1Gi`:: Specify the storage size for the cloned PVC. The storage size of the cloned PVC must be greater than or equal to the storage size of the source PVC.
--
