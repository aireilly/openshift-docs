...
+
--
`name: lvm-block-1-snap`:: Specify a name for the volume snapshot.
`persistentVolumeClaimName: lvm-block-1`:: Specify the name of the source PVC. {lvms} creates a snapshot of this PVC.
`volumeSnapshotClassName: lvms-vg1`:: Set this field to the name of a volume snapshot class. + [NOTE] ==== To get the list of available volume snapshot classes, run the following command: [source, terminal] ---- $ oc get volumesnapshotclass ---- ====
--
