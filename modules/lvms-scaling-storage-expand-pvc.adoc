...
+
--
`$ oc patch pvc <pvc_name> -n <application_namespace>`:: Replace `<pvc_name>` with the name of the PVC that you want to expand.
`--type=merge -p \ '{ "spec": { "resources": { "requests": { "storage": "<desired_size>" }}}}'`:: Replace `<desired_size>` with the new size to expand the PVC.
--
