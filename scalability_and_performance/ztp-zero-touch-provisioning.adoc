...
Zero touch provisioning (ZTP) addresses the problem of provisioning new telco sites from the moment there is network connectivity to the site and until the CNF workload is deployed and running on the site nodes.
...
Customers start by creating and refining ISO images of the node configuration they want. A unique image for each node is created based on the configuration provided. The images are then used to repeatedly provision large numbers of nodes efficiently and quickly, allowing the customer to keep up with the needs from the field for far edge nodes.
